...
export function useNotificationsController(): NotificationsController {
  ...
  const loadPage = useCallback(async (page: number, options: NotificationsLoadOptions = {}) => {
    ...
    try {
      ...
      if (__DEV__) {
        ...
        console.log('[worker-notifications] fetch-failed', loadError);
      }
  ...
  const markAllRead = useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__) {
        // eslint-disable-next-line no-console
        console.log('[worker-notifications] mark-all-read-failed', markError);
      }
    } finally {
  ...
  const openNotification = useCallback(async (item: NotificationListItem) => {
    if (!item.isRead) {
      ...
      try {
        ...
        if (__DEV__) {
          // eslint-disable-next-line no-console
          console.log('[worker-notifications] mark-read-failed', markError);
        }
      }
    }
  ...
  const deleteNotificationById = useCallback(async (item: NotificationListItem) => {
    ...
    try {
      ...
      if (__DEV__) {
        // eslint-disable-next-line no-console
        console.log('[worker-notifications] delete-failed', deleteError);
      }
    } finally {
